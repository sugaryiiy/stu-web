...
const backendResultText = computed(() =>
  backendResult.value ? JSON.stringify(backendResult.value, null, 2) : ''
)
...
const jsonPayload = computed(() => JSON.stringify({ text: jsonText.value }, null, 2))
...
const handleGenerate = async () => {
  ...
  try {
    ...
    backendResult.value = result?.jsonReq || result
    showToast('后端生成成功（JsonReq 已返回）')
  } catch (error) {
